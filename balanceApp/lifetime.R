...
dat_labels <- reactive({
  dat_raw() %>%
    ...
    arrange(Year) %>% 
    ungroup() %>% 
    add_row(`Career Stage` = "End", Year = max(dat_raw()$Year)) %>% 
...
# clean data
dat_cats2 <- reactive({
  data.frame(group = as_vector(dat_cats()[1, ]),
...
main_cols <- reactive({
  brewer_pal(palette = "Set2")(num_groups())
})

my_pal <- reactive({
  make_palette(num_groups(), main_cols(), num_cats_per_group())
})
